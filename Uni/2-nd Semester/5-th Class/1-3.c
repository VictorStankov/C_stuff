...
void writeToFile(char brand[], char model[], double engine_V, double price, char registered)
{
    ...
    FILE* binary = fopen("1-3.bin", "wb");
    if(binary == NULL)
        exit(1);
    FILE* text = fopen("1-3.txt", "w");
    if(text == NULL)
        exit(2);
...
{
    unsigned int sizeOfString;
    char model[5], registered;
	double engine_V, price;
    char everything[100];
    int i = 0;

    FILE* binary = fopen("1-3.bin", "rb");
    if(binary == NULL)
        exit(3);
    FILE* text = fopen("1-3.txt", "r");
    if(text == NULL)
        exit(4);

    // Same here
    fread(&sizeOfString, sizeof(unsigned int), 1, binary);
    char* brand = malloc(sizeOfString*sizeof(char));
